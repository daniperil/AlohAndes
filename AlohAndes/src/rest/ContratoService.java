...
public class ContratoService 
{
	...
	public Response createContrato(Contrato contrato) {
		AlohAndesTM tm = new AlohAndesTM(getPath());
		try {
			Contrato res = tm.createContrato(contrato);
	...
	public Response updateContrato(Contrato contrato)
	{
		AlohAndesTM tm = new AlohAndesTM(getPath());
		try {
			Contrato res = tm.updateContrato(contrato);
			return Response.status( 200 ).entity( res ).build();	
		}catch( Exception e )
		{
			return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
	...
	@Path("{IdContrato: \\d+}")
	@Produces( { MediaType.APPLICATION_JSON } )
	public Response borrarContrato(@PathParam("IdContrato") int idContrato)
	{
		AlohAndesTM tm = new AlohAndesTM(getPath());
		try {
			Contrato resp = tm.deleteContrato(idContrato);
