...
public class AlojamientoService 
{
		...
		@Produces({ MediaType.APPLICATION_JSON })
		public Response createAlojamiento(Alojamiento alojamiento) {
			AlohAndesTM tm = new AlohAndesTM(getPath());
			try {
				Alojamiento res = tm.createAlojamiento(alojamiento);
		...
		public Response updateAlojamiento(Alojamiento alojamiento)
		{
			AlohAndesTM tm = new AlohAndesTM(getPath());
			try {
				Alojamiento res = tm.updateAlojamiento(alojamiento);
				return Response.status( 200 ).entity( res ).build();	
			}catch( Exception e )
			{
				return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
