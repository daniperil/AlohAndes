	...
	//----------------------------------------------------------------------------------------------------------------------------------
			...
			public Response getSeguros() {
				...
				try {
					...
					seguros = tm.getAllSeguros();
					return Response.status(200).entity(seguros).build();
				} 
				catch (Exception e) {
			...
			@Path( "{id: \\d+}" )
			@Produces( { MediaType.APPLICATION_JSON } )
			public Response getSeguroById( @PathParam( "id" ) Long id )
			{
				try{
					AlohAndesTM tm = new AlohAndesTM( getPath( ) );
			...
			@Produces({ MediaType.APPLICATION_JSON })
			public Response createSeguro(Seguro seguro) {
				AlohAndesTM tm = new AlohAndesTM(getPath());
				try {
					Seguro res = tm.createSeguro(seguro);
			...
			public Response updateSeguro(Seguro seguro)
			{
				AlohAndesTM tm = new AlohAndesTM(getPath());
				try {
					Seguro res = tm.updateSeguro(seguro);
					return Response.status( 200 ).entity( res ).build();	
				}catch( Exception e )
				{
					return Response.status( 500 ).entity( doErrorMessage( e ) ).build( );
			...
			@Path("{IdSeguro: \\d+}")
			@Produces( { MediaType.APPLICATION_JSON } )
			public Response deleteSeguro(@PathParam("IdSeguro") int idSeguro)
			{
				AlohAndesTM tm = new AlohAndesTM(getPath());
				try {
